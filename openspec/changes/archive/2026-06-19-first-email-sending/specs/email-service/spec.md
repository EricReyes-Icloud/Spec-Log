...
The service MUST apply `withLineBreaks()` then `preparseMarkdown()` to newsletter markdown, render the result via the React Email `weekly-newsletter` template, and produce email-safe HTML using `@react-email/render`.

#### Scenario: Renders to valid HTML

- GIVEN newsletter with title and markdown content
